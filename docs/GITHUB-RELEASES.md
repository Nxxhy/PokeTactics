...
Abfragen verwenden den offiziellen stabilen `releases/latest`-Kanal und vergleichen die Tag-Version numerisch. Die von GitHub als „latest“ bezeichnete Veröffentlichung muss stets die höchste stabile Version sein; ältere Releases werden niemals als Downgrade installiert. Seit 9.1.2 gilt eine Versionspflicht: Beim Launcher-Start, alle 15 Minuten und unmittelbar vor jedem Spielstart wird GitHub online geprüft. Nur die exakt aktuelle stabile Version mit vollständigen, gültig signierten Update-Metadaten wird freigegeben. Normale Cache-Pausen ersetzen keine Startprüfung; eine online bestätigte ETag/304-Antwort ist zulässig. Fehlende Verbindung, ungültige oder unvollständige Releases sowie Download-/Installationsfehler sperren neue Spielstarts. Einminütige Fehlerpausen und persistentes Backoff bei GitHub-Abfragelimits bleiben verbindlich. Abgebrochene Pakete werden verworfen; „Erneut prüfen“ startet nach der Abfragepause einen neuen Versuch.

Installation erfolgt in einem neuen Versionsverzeichnis, anschließend wird `active.txt` atomar ersetzt. Bisherige Dateien bleiben erhalten; Fehler vor der Aktivierung lassen die bisherige Version aktiv, aber nicht zum Spielen freigegeben. Laufende Spiele werden auch bei Verbindungsverlust oder einem neuen Release nicht beendet. Der stabile Bootstrap startet den neuen Launcher; Launcher und .NET-Laufzeit werden im Update mitgeliefert. Der Bootstrap selbst wird durch den Installer aktualisiert. Benutzereinstellungen liegen außerhalb dieser Versionsverzeichnisse.

Die ausgelieferte Spiel-EXE führt direkte Starts zum Launcher zurück. Nach einer erfolgreichen Online-Prüfung übergibt dieser eine kurzlebige, einmalig verwendbare lokale Startfreigabe für die passende Version. Das verhindert versehentliche Umgehungen im regulären Startweg; es ist kein DRM gegen manipulierte Clients. Bereits heruntergeladene ältere Offline-Versionen können nicht nachträglich gesperrt werden. Für öffentliche Multiplayer-Partien bleibt eine serverseitige Versionskontrolle erforderlich; öffentliches Lobby-Hosting ist weiterhin nicht eingerichtet. Die frühere Offline-Zusage ist durch die ausdrücklich gewünschte Versionspflicht ersetzt.
